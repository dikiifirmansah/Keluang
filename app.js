...
// Flag: apakah dialog keluar sedang tampil
let _exitConfirmVisible = false;

(function initBackHandler() {
  history.pushState({ app: 'keluang', level: 0 }, '');

  window.addEventListener('popstate', () => {
    // Selalu push balik — mencegah browser benar-benar keluar
    history.pushState({ app: 'keluang', level: 0 }, '');

    // ── Prioritas 1: dialog keluar sedang tampil → user konfirmasi keluar
    ...
    // ── Prioritas 2: blur keyboard dulu (tanpa return) jika ada input aktif
    if (document.activeElement &&
        (document.activeElement.tagName === 'INPUT' ||
         document.activeElement.tagName === 'TEXTAREA')) {
      document.activeElement.blur();
      // Lanjut ke prioritas berikutnya, jangan return —
      // user sudah tahu mau keluar halaman, keyboard adalah efek samping.
    }

    // ── Prioritas 3: tutup crop overlay (wl / jurnal) — pakai .open
    const cropOverlay = document.querySelector('.wl-crop-overlay.open, #wlCropOverlay.open, #jrCropOverlay.open');
    if (cropOverlay) {
      cropOverlay.classList.remove('open');
      return;
    }

    // ── Prioritas 4: tutup onboarding jika sedang aktif
    const onbOverlay = document.getElementById('onbOverlay');
    if (onbOverlay && onbOverlay.classList.contains('active')) {
      // Navigasi mundur di dalam onboarding
      if (typeof onbBack === 'function') onbBack();
      return;
    }

    // ── Prioritas 5: tutup datepicker
    ...
    // ── Prioritas 6: tutup icon/category picker
    const pickerOverlay = document.getElementById('pickerOverlay');
    if (pickerOverlay && pickerOverlay.classList.contains('open')) {
      if (typeof closePicker === 'function') closePicker();
      return;
    }

    // ── Prioritas 7: tutup sheet / bottom-sheet manapun yang terbuka
    ...
    // ── Prioritas 8: tutup panel notif / period
    ...
    // ── Prioritas 9: navigasi balik ke halaman sebelumnya
    const activePage = document.querySelector('.page.active');
    ...
    // ── Sudah di beranda dan tidak ada yang terbuka → tanya konfirmasi keluar
    showExitConfirm();
  });
})();

// ── Helpers exit confirm
function _closeExitConfirm() {
  _exitConfirmVisible = false;
  const el = document.getElementById('exitConfirmOverlay');
  if (el) el.remove();
}
...
function showExitConfirm() {
  ...
  overlay.innerHTML = `
    ...
    ">
      ...
      <div style="display:flex; gap:10px;">
        <button onclick="_closeExitConfirm()" style="
  ...
  // Tap di luar dialog = batal
  overlay.addEventListener('click', e => {
    if (e.target === overlay) _closeExitConfirm();
  });

  document.body.appendChild(overlay);
  // TIDAK push state di sini — popstate berikutnya sudah ditangani via flag _exitConfirmVisible
}
