...
// ======================================================
// BACK BUTTON — native Android behavior untuk PWA
// ======================================================

// Peta navigasi: page aktif → tujuan back-nya.
// Nilai bisa berupa nama page (string → goTo) atau nama fungsi global
// (string yang match window[fn] → dipanggil langsung).
const PAGE_BACK_MAP = {
  'placeholder':      'beranda',
  'budget':           'beranda',
...
(function initBackHandler() {
  ...
  // Anchor pertama — supaya popstate selalu punya state sebelumnya,
  // sehingga app tidak langsung keluar saat back pertama kali ditekan.
  history.pushState({ app: 'keluang', level: 0 }, '');

  window.addEventListener('popstate', () => {

    // Segera push ulang — ini kunci agar app tidak pernah benar-benar exit
    // hanya karena browser mengonsumsi satu entry history.
    history.pushState({ app: 'keluang', level: 0 }, '');

    // ── Lapisan 1: Onboarding aktif ─────────────────────────────────────
    // Saat onboarding terbuka, back navigasi mundur antar step.
    // Jika sudah di welcome page (step awal), back diabaikan — user
    // harus menyelesaikan onboarding dulu sebelum bisa pakai app.
    const onbOverlay = document.getElementById('onbOverlay');
    if (onbOverlay && onbOverlay.classList.contains('active')) {
      onbBack();
      return;
    }

    // ── Lapisan 2: Dialog konfirmasi keluar ──────────────────────────────
    // Jika dialog exit sedang tampil, back = batalkan (tutup dialog).
    const exitDialog = document.getElementById('exitConfirmOverlay');
    if (exitDialog) {
      exitDialog.remove();
      return;
    }

    // ── Lapisan 3: Crop overlay ──────────────────────────────────────────
    // Crop overlay pakai class wl-crop-overlay, bukan sheet-overlay,
    // jadi perlu dicek terpisah.
    const wlCrop = document.getElementById('wlCropOverlay');
    if (wlCrop && wlCrop.classList.contains('open')) {
      wlCrop.classList.remove('open');
      return;
    }
    const jrCrop = document.getElementById('jrCropOverlay');
    if (jrCrop && jrCrop.classList.contains('open')) {
      jrCrop.classList.remove('open');
      return;
    }

    // ── Lapisan 4: Sheet / bottom sheet / picker / modal ─────────────────
    // Semua overlay dengan class sheet-overlay yang sedang .open ditutup
    // satu per satu dari yang paling atas (querySelector ambil pertama di DOM,
    // tapi karena overlay ditumpuk via z-index, yang terakhir dibuka biasanya
    // yang paling atas — ambil yang terakhir agar sesuai tumpukan visual).
    const allOpenSheets = [...document.querySelectorAll('.sheet-overlay.open')];
    if (allOpenSheets.length > 0) {
      const topSheet = allOpenSheets[allOpenSheets.length - 1];
      closeSheet(topSheet.id !== 'sheetOverlay' ? topSheet.id : undefined);
      return;
    }

    // ── Lapisan 5: Panel slide-in (notif & periode) ──────────────────────
    ...
    // ── Lapisan 6: Tutup keyboard (blur input aktif) ─────────────────────
    // Jika ada input/textarea yang fokus, back tutup keyboard dulu
    // sebelum melakukan navigasi — persis seperti native Android.
    ...
    // ── Lapisan 7: Navigasi antar screen ────────────────────────────────
    // Ikuti PAGE_BACK_MAP — kalau nilainya nama fungsi global, panggil
    // fungsinya; kalau nama page, pakai goTo().
    const activePage = document.querySelector('.page.active');
    ...
    // ── Lapisan 8: Root screen — konfirmasi keluar ───────────────────────
    // Sudah di beranda, tidak ada yang terbuka → tampilkan dialog keluar.
    showExitConfirm();
  });
...
// Double-tap back: back pertama → dialog muncul, back kedua → dialog hilang
// (ditangani Lapisan 2 di atas). User harus tap tombol "Keluar" untuk exit.
