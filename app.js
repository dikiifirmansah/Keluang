      ...
      listEl.innerHTML = list.map(s => `
        <div class="picker-item" onclick="selectPickerItem('${mode}','${s.id}')">
          <div class="picker-item-icon" style="background:${sourceTypeColors[s.jenis]}; border-radius:10px;">
            <i class="bi ${sourceIcons[s.jenis] || 'bi-wallet2'}" style="color:#fff; font-size:16px;"></i>
          </div>
          <div>
...
function selectPickerItem(mode, val) {
  if (mode === 'category') {
    ...
    if (s) {
      const iconHtml = `<i class="bi ${sourceIcons[s.jenis] || 'bi-wallet2'}"></i>`;
      ['txSourceIconWrap','txSourceIconWrapTransfer'].forEach(id => {
        const el = document.getElementById(id);
        el.innerHTML = iconHtml;
        el.style.background = sourceTypeColors[s.jenis] || 'var(--ink-300)';
    ...
    if (s) {
      ...
      iconWrap.innerHTML = `<i class="bi ${sourceIcons[s.jenis] || 'bi-wallet2'}"></i>`;
...
(function initBackHandler() {
  ...
  window.addEventListener('popstate', e => {
    ...
    // Sudah di halaman utama, tidak ada yang terbuka — diam saja
  });
})();
