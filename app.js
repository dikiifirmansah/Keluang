...
(function initBackHandler() {
  ...
  window.addEventListener('popstate', e => {
    // Langsung push state baru — ini yang menjaga app tidak keluar
    history.pushState({ app: 'keluang', level: 0 }, '');

    // Prioritas 1: tutup dialog keluar kalau sedang terbuka
    const exitOverlay = document.getElementById('exitConfirmOverlay');
    if (exitOverlay) { exitOverlay.remove(); return; }

    // Prioritas 2: tutup sheet yang terbuka
    ...
    // Prioritas 3: tutup panel yang terbuka
    ...
    // Prioritas 4: blur input aktif (tutup keyboard)
    if (document.activeElement && (document.activeElement.tagName === 'INPUT' || document.activeElement.tagName === 'TEXTAREA')) {
      document.activeElement.blur(); return;
    }

    // Prioritas 5: navigasi balik ke halaman sebelumnya
    const activePage = document.querySelector('.page.active');
    ...
    // Sudah di beranda, tidak ada yang terbuka — tampilkan konfirmasi keluar
    // Kalau dialog sudah terbuka, berarti user confirm keluar
    if (document.getElementById('exitConfirmOverlay')) {
      document.getElementById('exitConfirmOverlay').remove();
...
function showExitConfirm() {
  ...
  // Push state langsung saat dialog muncul, bukan nunggu Batal
  history.pushState({ app: 'keluang', level: 0 }, '');
}
